...
public class addProductionCompany {
    ...
    public void addProductionCompanies(TmdbMovies tmdbMovies, int finalI, MovieDb movie, List<String> batchQueries) throws IOException {
        ...
        try {
            ...
            for (int i = 0; i < productionCompany.size(); i++) {
                ...
                batchQueries.add("MATCH (m:Movie {title: '" + stringFixer.fixString(movie.getTitle()) + "'}), (p:Company {name: '" +companyName+ "'}) MERGE (m)-[:PRODUCED_BY]->(p)");
